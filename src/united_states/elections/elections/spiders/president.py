...
class PresidentSpider(scrapy.Spider):
    ...
    start_urls = [f"https://api.open.fec.gov/v1/candidates/?page=1&per_page=100&is_active_candidate=true&election_year=2024&office=P&candidate_status=C&sort=name&sort_hide_null=false&sort_null_only=false&sort_nulls_last=false&api_key={API_KEY_FEC}"]

    def parse(self, response):
        data = json.loads(response.body)
        candidate_ids = [item['candidate_id'] for item in data['results']]
        need_ids = ReadArticles().check_batch_id(candidate_ids, 'president_candidates', 'candidate_id')
        for item in data['results']:
            if item['candidate_id'] in need_ids:
                yield { 
                    ...
                    'state' : item["state"],
                    'collection_name': 'president_candidates'
                }
            ...
            if pagination:
                if pagination['page'] < pagination['pages']:
                    ...
                    current_page += 1
                    url = f"https://api.open.fec.gov/v1/candidates/?page={current_page}&per_page=100&is_active_candidate=true&election_year=2024&office=P&candidate_status=C&sort=name&sort_hide_null=false&sort_null_only=false&sort_nulls_last=false&api_key={API_KEY_FEC}"
