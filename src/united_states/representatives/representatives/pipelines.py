...
class RepresentativesPipeline:
    def process_item(self, item, spider):
        adapter = ItemAdapter(item)
        field_names = adapter.field_names()
        for field_name in field_names:
            if field_name == 'president':
                value = adapter.get(field_name).replace('Documents signed by ','').replace(', of type Presidential Document, and of presidential document type Executive Order', '')
                adapter[field_name] = value
            if field_name in ['created_at', 'signing_date']:
                value = adapter.get(field_name).rstrip().lstrip()
                adapter[field_name] = str(dateutil.parser.parse(value))

            # if field_name == 'title':
            #     split_string = adapter.get(field_name).split("—")
            #     int(split_string[0].strip())
            #     adapter['executive_order_number'] = (split_string[0].strip())
            #     adapter[field_name] = split_string[1].strip()
        return item
...
class WriteRespresetativesArticles:
