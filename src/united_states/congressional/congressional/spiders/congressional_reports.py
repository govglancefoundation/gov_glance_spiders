...
class CongressionalReportsSpider(scrapy.Spider):
    name = "congressional_reports"
    allowed_domains = ["api.govinfo.gov"]
    start_urls = [f"https://api.govinfo.gov/collections/CRPT/2024-01-01T06%3A25%3A40Z?pageSize=100&offsetMark=%2A&api_key={GOV_INFO_API_KEY}"]

    def parse(self, response):
        
        data = json.loads(response.body)
        for package in data['packages']:
            scrapped = ReadArticles().check_item('congressional_reports', package['packageId'], package['lastModified'])
            if scrapped == False:
    ...
    def parse_item(self, response):
        ...
        item = {
            ...
            'collection_name': 'Congressional Reports',
