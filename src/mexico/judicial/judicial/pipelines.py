    ...
    def process_item(self, item, spider):
        ...
        for field_name in field_names:
            ...
            if field_name in ['description', 'title']:
                if isinstance(adapter.get(field_name), list):
                    value = adapter.get(field_name)
                    edit = [string.replace('• ', '').replace("\n", "").lstrip().rstrip() for string in value]
                    joined = ' '.join(edit).rstrip().lstrip()
                    adapter[field_name] = re.sub(r'\s+', ' ', joined)
